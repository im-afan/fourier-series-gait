...
class Generator(nn.Module):
    def __init__(
        ...
        self.c = nn.Parameter(torch.ones((n_means)))
        self.u = nn.Parameter(torch.rand((n_means, 2*action_size*self.n_frequencies+1)))
        self.o = nn.Parameter(torch.rand((n_means, 1)))
        self.register_parameter("means", param=self.u)
        self.register_parameter("std", param=self.o)
        self.register_parameter("weight", param=self.c)
    ...
    def entropy(self):
        ...
        for i in range(self.n_means):
            normal = D.Normal(self.u[i], self.o[i])
            normals.append(normal)
            h_xc += self.c[i] * normal.entropy()
        ...
        for i in range(self.n_means):
            ...
            for j in range(self.n_means):
                #print(-D.kl_divergence(normals[i], normals[j]))
                divergence = D.kl_divergence(normals[i], normals[j])
                sum += self.c[j] * torch.exp(-torch.norm(divergence))
            entropy -= self.c[i] * torch.log(sum)
        print("entropy calc time: {}".format(time.time()-start))
        return entropy

    def sample_gait(self, n=1):
        c_normalized = self.c / self.c.norm()
        mix = D.Categorical(c_normalized)
        comp = D.Independent(D.Normal(self.u, self.o), 1)
...
class Trainer:
    ...
    def train(self, epochs=1000, batch_size=32, val_batch_size=100, value_samples=32):
        ...
        for i in range(epochs):
            ...
            means, std = self.generator()
            simulate, log_prob = self.generator.sample_gait(n=batch_size)
            entropy = self.generator.entropy()
            #print(means.shape)
            value = torch.mean(self.value(means.detach()))
            #print(self.normalize_reward(value, inverse=True))
            value_detached = value.detach()

            np.savetxt("saved_agents/" + str(i) + ".npy", simulate[0].detach().numpy())
